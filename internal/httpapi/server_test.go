...
func TestCatalogSurfacesMountOnlyConfiguredPaths(t *testing.T) {
	...
	paths := []string{
		...
		"/api/v1/admin/routes/route-id/versions",
		"/api/v1/admin/routes/preview",
	}
	...
	if modelCalls != 1 || adminCalls != 8 {
		t.Fatalf("catalog handler calls = %d/%d, want model:1 admin:8", modelCalls, adminCalls)
	}
