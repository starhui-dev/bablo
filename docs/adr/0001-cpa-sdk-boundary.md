...
- Bablo 定义自己的 `InferenceEngine`、`InferenceRequest`、`ExecutionResult`、`Stream`、`Capabilities`、`ResolvedRoute`、`UpstreamError`；
...
- adapter 负责生命周期、能力探测、协议/错误/stream/cancel/request ID 映射；CPA usage queue 只能是观测/reconcile 输入，不能是账本；
...
- adapter 以 fake provider 测试 non-stream/stream/cancel/429/401/5xx/refresh/shutdown；
